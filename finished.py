...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Parameters do not match values given")
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Incorrect number of arguments for {0}".format(self.name))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        vals = Pair(expressions.first.second, expressions.second)
        env.define(expressions.first.first, do_lambda_form(vals, env))
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if first.second is not nil:
                return eval_all(first.second, env)
            elif first.first == 'else':
                return eval_all(first.second, env)
            else:
                return test
